(function(){
  ...
  var Sloth = SlothVolleyball.Sloth = function(isLeft, pos, game) {
    ...
    this.img = new Image();
    if(this.isLeft){
      this.img.src = 'images/lefty.png';
    } else {
      this.img.src = 'images/righty.png';
    };
  ...
  Sloth.prototype.draw = function(ctx) {
		...
		// ctx.fill(path);
    ctx.drawImage(this.img, this.pos.x - this.rad, this.pos.y - this.rad, 160, 80);
  ...
  Sloth.prototype.collideWithNet = function(){
    ...
    if(this.isLeft){
      ...
      this.pos.x = this.game.net.pos_x + this.game.net.width + this.rad;
    }
  }
